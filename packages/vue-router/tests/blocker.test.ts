...
async function setup({ blockerFn, disabled, ignoreBlocker }: BlockerTestOpts) {
	...
	const rootRoute = createRootRoute()
		...
		const indexRoute = createRoute({
			...
			path: '/',
			// Use a plain setup-function component so it matches RouteComponent signature
			component: (props?: any) => {
				const navigate = useNavigate()
				useBlocker({ disabled, shouldBlockFn: _mockBlockerFn })
				return h('div', [
					h('h1', 'Index'),
					h(Link, { to: '/posts', ...(ignoreBlocker ? { ignoreBlocker } : {}) }, 'link to posts'),
					h(Link, { to: '/foo' }, 'link to foo'),
					h('button', { onClick: () => navigate({ to: '/posts', ignoreBlocker }) }, 'button'),
				])
			},
		})
		...
		const postsRoute = createRoute({
			getParentRoute: () => rootRoute,
			path: '/posts',
			component: (props?: any) => h('h1', 'Posts'),
		})
		...
		const fooRoute = createRoute({
			...
			component: (props?: any) => h('h1', 'Foo'),
		})

		const barRoute = createRoute({
			getParentRoute: () => rootRoute,
			path: '/bar',
			component: (props?: any) => h('h1', 'Bar'),
		})
	...
	// Render the RouterProvider as the root component
