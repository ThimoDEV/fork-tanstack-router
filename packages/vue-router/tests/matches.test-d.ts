...
test('when matching a route with params', () => {
  ...
  // Infer the options type for matchRoute and assert the 'to' prop is a string-like path
  type MRParam = Parameters<typeof matchRoute<string, '/invoices/$invoiceId'>>[0]
  expectTypeOf<MRParam['to']>().toMatchTypeOf<string>()

  expectTypeOf(
    matchRoute({
      to: '/invoices/$invoiceId',
    }),
  ).toEqualTypeOf<ComputedRef<false | { invoiceId: string }>>()
})
...
test('when matching a route with params underneath a layout route', () => {
  ...
  ).toEqualTypeOf<ComputedRef<false | { id: string }>>()
})
