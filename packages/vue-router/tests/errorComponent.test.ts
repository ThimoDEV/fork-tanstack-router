...
function MyErrorComponent(props: any) {
	return h('div', `Error: ${props.error.message}`)
}
			...
			test.each(callers)(
				...
				async ({ caller, testFn }) => {
					...
					const indexRoute = createRoute({
						...
						path: '/',
						component: () => h('div', [h(Link, { to: '/about' }, 'link to about')]),
					})
					const aboutRoute = createRoute({
						...
						component: () => h('div', 'About route content'),
					...
					const errorComponent = await screen.findByText('Error: error thrown', undefined, {
						timeout: 1500,
					} as any)
			...
			test.each(callers)(
				...
				async ({ caller, testFn }) => {
					...
					const indexRoute = createRoute({
						...
						beforeLoad: caller === 'beforeLoad' ? testFn : undefined,
						loader: caller === 'loader' ? testFn : undefined,
						component: () => h('div', 'Index route content'),
						errorComponent: MyErrorComponent,
					})
